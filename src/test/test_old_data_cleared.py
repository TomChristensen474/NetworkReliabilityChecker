...
class TestClearData(unittest.TestCase):
    def test_clear_old_data(self):
        ...
        if connection:
            ...
            cursor.execute(
                "INSERT INTO network_history VALUES(NULL, ?, 100, 0.3, 0)",
                (date_time_in_date,),
            )
            cursor.execute(
                "INSERT INTO network_history VALUES(NULL, ?, 100, 0.3, 0)",
                (date_time_out_of_date,),
            )
            connection.commit()

            data = bar_chart.get_data_from_DB(
                200, connection
            )  # get data from last 200 hours (i.e. just over a week)
